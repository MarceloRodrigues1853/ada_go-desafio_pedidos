  ...
  return (
    <div className="shell">
      ...
      <main>
        ...
        {view === 'pedidos' && <OrdersView orders={orders} clients={clients} products={products} onCreate={(data) => mutate(() => api.orders.create(data), data.simulation_outcome === 'APPROVED' ? 'Pedido criado. A aprovação será processada pela Saga.' : 'Pedido criado. A recusa será processada e o estoque devolvido.')} onPay={(id) => mutate(() => api.orders.pay(id), 'Pagamento solicitado.')} onCancel={(id) => mutate(() => api.orders.cancel(id), 'Pedido cancelado.')} />}
      </main>
    </div>
  )
}
